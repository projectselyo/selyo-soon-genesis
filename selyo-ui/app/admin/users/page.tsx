...
const UserList: React.FC = () => {
  ...
  // Combined Conversion and Reversal
  const convertAndReverse = () => {
    const decimalValue = parseInt(newUid, 10);
    ...
    if (hexValue.length % 2 !== 0) {
      alert("Converted UID must have an even number of characters.");
      return;
    }

    // Reverse the hexadecimal string by two characters
    const reversedUid = hexValue
      .match(/.{1,2}/g)
      ?.reverse()
      .join("");
    if (reversedUid) {
      setNewUid(reversedUid);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100">
      <nav className="bg-gradient-to-r from-sky-600 to-emerald-300 p-4">
        <div className="container mx-auto flex justify-between items-center">
          <div className="text-white text-lg font-bold">User Directory</div>
        </div>
      </nav>
      <div className="container mx-auto py-8">
        <h2 className="text-2xl font-semibold mb-4 text-neutral-800">Users</h2>
        <input
          type="text"
